...
class UserChat extends Model
{
    ...
    public static function sendText($data)
    {
        ...
        $data = json_encode($data) . "\n";//(is_scalar($data) ? strval($data) : json_encode($data)) . "\n";
        try {
            //长连接不建议关闭连接
    ...
    public static function getSocket($force_connect = false)
    {
        ...
        if (self::$_socket_client == null || $force_connect) {
            self::$_socket_client = stream_socket_client('tcp://' . self::$config['worker_push_url'] . ':' . 2003, $errno, $errstr);//self::$config['text_worker_port']
        }
